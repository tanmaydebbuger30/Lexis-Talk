...
if len(chunks) == 0:
    print("⚠️ WARNING: No chunks found! Check if PDFs were loaded correctly.")
...
def rag_advanced(
    ...
    if not retrieved:
        print("⚠️ WARNING: No documents retrieved from RAG!")
...
def synthesize_tts_gpt(text: str, output_path: str = None, voice: str = "alloy") -> str:
    ...
    try:
        ...
        if len(text) > 4000:
            print(f"⚠️ Text truncated from {len(text)} to 4000 chars for TTS")
...
def gradio_rag_text(user_text, input_lang_choice, output_lang_choice, history):
    ...
    if not text:
        history.append({"role": "user", "content": "⚪️ Empty message"})
